...
void Delay(int n);
void updateTime();
float updateFunction(float time);
...
void Initialize(void){
	...
	deformedVertices = std::vector<glm::vec3>(vertices);
...
void Display(void)
{
	...
	glBindVertexArray(vao);
	///*
	float ft = updateFunction(timeUpdate);
	//printf("ft: %f\n", ft);4

	//Here you need to update vertices using either glMapBuffer of glBufferSubData
	for (int i = 0; i < vertices.size();i++)
	{
		deformedVertices[i] = vertices[i] + ft*normals[i];
	}
	
	
	glBindBuffer(GL_ARRAY_BUFFER, vbo[0]);
	glBufferSubData(GL_ARRAY_BUFFER, 0, deformedVertices.size()*sizeof(glm::vec3), &deformedVertices[0]);
...
float updateFunction(float time)
{
	return amplitude * glm::sin((TWOPI * frequency * time) + phase);
}
/**************************************************************************************/
