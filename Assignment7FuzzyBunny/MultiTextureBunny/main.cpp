...
void Initialize(void){
	...
	bunny = new Bunny(loader);
	bunny2 = new Bunny(loader);
	// Use our shader

	view = glm::lookAt(vec3(0.0f, 0.0f, 2.0f), vec3(0.0f, 0.0f, 0.0f), vec3(0.0f, 1.0f, 0.0f));
	projection = mat4(1.0f);
	program = LoadShaders("multitex.vs", "multitex.fs", "geometry.gs");
	program2 = LoadShaders("multitex2.vs", "multitex.fs");

	vec3 light_intensity(1.0f, 1.0f, 1.0f);
	vec4 light_position(10.0f, 10.0f, 10.0f, 1.0f);

	vec3 material_ambient(0.0, 0.0, 1.0);
	vec3 material_diffuse(0.0, 0.0, 1.0);
	vec3 material_specular(0.0, 0.0, 1.0);
	...
	///*
	vec3 material_ambient2(0.9, 0.5, 0.3);
	vec3 material_diffuse2(0.9, 0.3, 0.3);
	vec3 material_specular2(0.8, 0.8, 0.8);
	
	glUseProgram(program2);
	glUniform3fv(glGetUniformLocation(program2, "Light.Intensity"), 1, (GLfloat*)&light_intensity);
	glUniform4fv(glGetUniformLocation(program2, "Light.Position"), 1, (GLfloat*)&light_position);
	glUniform3fv(glGetUniformLocation(program2, "Material.Ka"), 1, (GLfloat*)&material_ambient2);
	glUniform3fv(glGetUniformLocation(program2, "Material.Kd"), 1, (GLfloat*)&material_diffuse2[0]);
	glUniform3fv(glGetUniformLocation(program2, "Material.Ks"), 1, (GLfloat*)&material_specular2[0]);
	glUniform1f(glGetUniformLocation(program2, "Material.Shininess"), shininess);
	//*/
...
void Display(void)
{
	...
	///*
	glUseProgram(program2);
	glUniformMatrix4fv(glGetUniformLocation(program2, "ModelViewMatrix"), 1, GL_FALSE, (GLfloat*)&model_view[0]);
	glUniformMatrix3fv(glGetUniformLocation(program2, "NormalMatrix"), 1, GL_FALSE, (GLfloat*)&normalmatrix[0]);
	glUniformMatrix4fv(glGetUniformLocation(program2, "MVP"), 1, GL_FALSE, (GLfloat*)&mvp[0]);
	glUniformMatrix4fv(glGetUniformLocation(program2, "ProjectionMatrix"), 1, GL_FALSE, (GLfloat*)&projection[0]);
	bunny2->render();
	glBindVertexArray(0);
	//*/
